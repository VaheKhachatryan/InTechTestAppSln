...
{
	public class RedisCache : IDistributedCache, IExtendedDistributedCache, IDisposable
	{
		// KEYS[1] = key
		...
		public bool KeyExists(string key, bool refresh = false)
		{
			if (key == null)
			{
				throw new ArgumentNullException(nameof(key));
			...
			var isExists = _cache.KeyExists(key);
		...
		public async Task<bool> KeyExistsAsync(string key, bool refresh = false, CancellationToken token = default)
		{
			if (key == null)
			{
				throw new ArgumentNullException(nameof(key));
			...
			var isExists = await _cache.KeyExistsAsync(key);
		...
		public void RemoveByPattern(string pattern)
		{
			...
			foreach (var endPoint in this._connection.GetEndPoints())
			{
				var server = this._connection.GetServer(endPoint);
				var keys = server.Keys(this._cache.Database, pattern: $"{_instance}*{pattern}*");
				this._cache.KeyDelete(keys.ToArray());
			}
		}
		...
		public async Task RemoveByPatternAsync(string pattern, CancellationToken token = default)
		{
			...
			foreach (var endPoint in this._connection.GetEndPoints())
			{
				var server = this._connection.GetServer(endPoint);
				var keys = server.Keys(this._cache.Database, pattern: $"{_instance}*{pattern}*");
				await this._cache.KeyDeleteAsync(keys.ToArray()).ConfigureAwait(false);
			}
		}
		...
		public async Task ClearAllKeysAsync(CancellationToken token = default)
		{
			await ConnectAsync(token).ConfigureAwait(false);
		...
		public async Task RefreshAbsoluteExpiryAsync(string key, TimeSpan absExpr, CancellationToken token = default(CancellationToken))
		{
			if (key == null)
			{
				throw new ArgumentNullException(nameof(key));
			...
			await _cache.KeyExpireAsync(_instance + key, absExpr);
		}
	}
}